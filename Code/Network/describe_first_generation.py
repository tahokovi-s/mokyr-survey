...
Outputs:
  ...
  Data/Derived/First_Generation_Respondent_Profile_{date}.csv
  Data/Derived/First_Generation_Excluded_{date}.csv
  Output/First_Generation_Descriptives_{date}.md

Universes:
  - ALL gen-1: used for network/graph-based statistics
  - RESPONDENT gen-1: used for survey-field statistics
"""
...
def write_respondent_profile(gen1_resp, out_path: Path):
    """Aggregated distributions for respondent gen-1 only."""
    rows: list[dict] = []
    ...
    years = [int(n["phd_year"]) for n in gen1_resp if n["phd_year"].strip()]
    if years:
    ...
    # --- PhD institution (canonical, respondent-only) ---
    inst_counter = Counter(
        n["phd_institution_canon"] for n in gen1_resp
    ...
    # --- Current employer (canonical, respondent-only) ---
    emp_counter = Counter(
        n["current_employer_canon"] for n in gen1_resp
        if n["current_employer_canon"].strip()
    )
    emp_blank = sum(1 for n in gen1_resp if not n["current_employer_canon"].strip())
    ...
    # --- Has-students by PhD-year bin (ecma recommendation) ---
...
def write_markdown(gen1_all, gen1_resp, gen1_nonresp, subtree_stats,
    ...
    if discrepancy_available:
        _p(
            f"Primary Gen-1 descriptives use the strict final network universe only. "
            f"{exclusion_summary['total']} respondent cases were excluded from Gen-1 headline counts "
        ...
        if exclusion_rows:
            _p("| Reason | Count |")
            _p("|---|---|")
            for category, count in sorted(exclusion_summary["by_category"].items(), key=lambda item: (_category_label(item[0]), item[0])):
    ...
    # --- Section 2: Respondent-only facts ---
    _h(2, "Respondent First-Generation Students (Survey Universe)")
    _blank()
    _p(f"Universe: {n_resp} respondent generation-1 nodes. Statistics below use "
       f"survey fields that are only available for respondents.")
    _blank()
    ...
    _blank()
    _p(f"- N with year: {len(years)} / {n_resp}")
    ...
    _blank()
    _p(f"- N with institution: {sum(inst_counter.values())} / {n_resp}")
    ...
    _blank()
    _p(f"- N with employer: {sum(emp_counter.values())} / {n_resp}")
    ...
    _h(3, "Country")
    _blank()
    _p(f"- N with country: {sum(country_counter.values())} / {n_resp}")
    _blank()
    _p("| Country | Count |")
    _p("|---|---|")
    for c, cnt in country_counter.most_common():
    ...
    # US state
    state_counter = Counter(
        n["us_state"] for n in gen1_resp if n["us_state"].strip()
    )
    _h(3, "US State")
    _blank()
    us_resp = sum(1 for n in gen1_resp
                  if n["country"].strip().startswith("United States"))
    _p(f"- US-based respondents: {us_resp}")
    ...
    # Has-students
    _h(3, "Has Supervised PhD Students")
    _blank()
    ...
    _p(f"3. **Non-respondent data is sparse.** The {n_nonresp} non-respondent gen-1 "
       "nodes lack survey fields (PhD year, institution, employer, country). "
       f"Respondent-only statistics use N={n_resp} as denominator.")
    _p("4. **Canonical institution/employer mappings are incomplete.** "
    ...
    if discrepancy_available:
        _p(
            f"5. **{validation_summary['blank_generation_count']} respondent nodes have no assigned generation** overall. "
            f"Of these, {exclusion_summary['total']} appear in the Gen-1 exclusion appendix and are excluded from headline Gen-1 counts."
        )
    else:
        _p(f"5. **{validation_summary['blank_generation_count']} respondent nodes have no "
           "assigned generation** and are excluded "
           "from generation-1 counts. See validation report for details.")
    _p("6. **Generation assignments use topology-corrected values.** "
...
def main():
    ...
    profile_path = out_dir / f"First_Generation_Respondent_Profile_{date_token}.csv"
    ...
    write_respondent_profile(gen1_resp, profile_path)
