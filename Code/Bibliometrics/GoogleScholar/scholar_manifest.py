...
from Bibliometrics.GoogleScholar.schemas import validate_manifest_record
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate Google Scholar enrichment manifests for Mokyr scholars")
...
def build_search_queries(scholar: dict[str, Any]) -> list[str]:
    ...
    for variant in name_variants:
        queries.append(f"{variant} Google Scholar")

    if full_name:
        queries.append(f"{full_name} Google Scholar")
        if institution:
            queries.append(f"{full_name} {institution} economics")

    folded_full_name = strip_diacritics(full_name)
    if folded_full_name and folded_full_name != full_name:
        queries.append(f"{folded_full_name} Google Scholar")
        if institution:
            queries.append(f"{folded_full_name} {institution} economics")
...
def main() -> None:
    ...
    output_default = PROJECT_ROOT / "Data" / "Derived" / f"GS_Manifest_{date_token}.jsonl"
