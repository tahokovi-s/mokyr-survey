...
from Bibliometrics.GoogleScholar.schemas import validate_enrichment_record, validate_manifest_record
...
GS_MANIFEST_PATTERN = re.compile(r"^GS_Manifest_(\d{6})\.jsonl$")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate Google Scholar enrichment JSONL output")
    parser.add_argument("--date", default=None, help="Date suffix for default input/output paths (MMDDYY)")
    parser.add_argument("--manifest", default=None, help="Path to GS manifest JSONL")
...
def infer_date_token(args: argparse.Namespace) -> str:
    if args.date:
        return args.date
    if args.manifest:
        match = GS_MANIFEST_PATTERN.match(Path(args.manifest).name)
        if match:
...
def cross_check_name(manifest: dict[str, Any], enrichment: dict[str, Any]) -> str:
    ...
    profile_name = (google_scholar.get("name_on_profile") or "").strip()
    if not profile_name:
        return "skip"
...
def cross_check_affiliation(manifest: dict[str, Any], enrichment: dict[str, Any]) -> str:
    google_scholar = enrichment.get("google_scholar") or {}
    profile_affiliation = (google_scholar.get("affiliation_on_profile") or "").strip()
...
def check_temporal_plausibility(manifest: dict[str, Any], enrichment: dict[str, Any]) -> str:
    ...
    if isinstance(h_index, int) and h_index > 5 * years_since_phd:
        return "fail"
    if isinstance(cited_by_count, int) and cited_by_count > 50 * years_since_phd:
        return "fail"
    return "pass"
...
def check_source_urls(enrichment: dict[str, Any]) -> str:
    if enrichment.get("lookup_status") != "found":
        return "skip"
    google_scholar = enrichment.get("google_scholar") or {}
    url = (google_scholar.get("url") or "").strip()
    if not url:
        return "fail"
...
def validate_one(
    ...
    *,
    ...
    google_scholar = working.get("google_scholar") or {}
    return {
        "node_id": working.get("node_id", ""),
        "full_name": full_name,
        "generation": generation,
        "lookup_status": working.get("lookup_status", ""),
...
def main() -> None:
    ...
    manifest_default = PROJECT_ROOT / "Data" / "Derived" / f"GS_Manifest_{date_token}.jsonl"
    output_default = PROJECT_ROOT / "Data" / "Derived" / f"GS_Validation_{date_token}.csv"
